...
void *test_case_read(void *v)
{
	...
	while(1)
	{
		...
		//link_list_remove_first
		link_list_remove_first(&link_list,fpcap_node_t);

		//link_list_remove_last
		//link_list_remove_last(&link_list);
		//fpcap_node_t *np = container_of(link_list.last,fpcap_node_t,node);
		//free(np);
...
void *test_case_write(void *v)
{
	...
	while(1)
	{
		k=pthread_mutex_lock(&(client->lock));
...
void test_init(client_t	*client)
{
	...
	max_cases = 10;
	//初始化读写锁
	k=pthread_mutex_init(&(client->lock), NULL);
	debug("%d",k);
