...
main(int argc, char *argv[])
{
	...
	if((have_conf=conf_load("/etc/agent/agent.cnf"))!=0)
		have_conf=conf_load("/opt/agent/etc/agent.cnf");
	...
	//线程池
	thpool_init(&thpool,2,1);
	usleep(50);

	const char *master_enable = conf_get(CONF_MASTER_KEY);
	const int port=SLAVE_PORT;
	if(!strcmp(master_enable,"Yes"))
	{
		if(thpool_add_work(&thpool,fserver_pthread,(void *)&port)<0)
		{
			debug("thpool full");
		}
	}
...
#define OPTSTRING "+2" OPTSTRING_CAPTURE_COMMON OPTSTRING_DISSECT_COMMON "M:C:e:E:F:gG:hH:j:J:lo:O:PqQr:R:S:T:U:vVw:W:xX:Y:z:"
	...
	//fprintf(stdout, "%s\t%d\t[DEBUG]:optarg %c\n",__FILE__,__LINE__,opt);
	switch (opt) {
...
report_counts(void)
{
	...
	debug("report_counts infodelay：%d",infodelay);
#endif /* SIGINFO */
}
